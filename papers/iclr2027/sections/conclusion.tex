...
ODE formulation has source-backed stability and spectral-bound regression coverage in
both tests and the local reproducibility harness.  We emphasize that ODE stability is
necessary but not sufficient for correct routing: the trained dynamics must be directed
toward useful specialization patterns via task performance feedback.
...
\begin{itemize}
  ...
  \item \textbf{Monotonic improvement auditing.}  The \texttt{EvolutionLog} component
    (new in the package) enforces strict monotonicity and strict acceleration on
    governance metric time series at write time via SQLite triggers.  Integrating it
    into SWE-bench training loops would provide a formal, structural guarantee that
    task success improves epoch-by-epoch --- any regression or deceleration is rejected
    before being logged, turning the audit trail into a safety contract rather than a
    post-hoc check.
\end{itemize}
...
The \texttt{SpectralSphereManifold} and \texttt{swarm\_ode} regression tests plus
\texttt{scripts/reproduce\_paper\_claims.py} reproduce the reported local claims.
Official external SWE-bench results are not claimed in this submission.
