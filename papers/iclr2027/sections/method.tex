...
\begin{theorem}[SpectralSphere Prevents Collapse]
  ...
  \]
  where $V_\infty = \alpha^2 \cdot \mathrm{Var}(I)$ is the variance of the residual
  attractor.  In particular, BUC does not occur.
\end{theorem}
...
\begin{theorem}[Lyapunov Stability of Trust ODE]
  ...
  \]
  satisfies $\dot{V}(H) \leq -\alpha V(H)$ along trajectories, establishing global
...
\paragraph{Topological capacity.}
The Neural ODE achieves dramatically higher topological capacity than Sinkhorn.  We
measure capacity as the volume (Lebesgue measure) of the reachable set in matrix space
after 10 cycles, normalized by the Sinkhorn baseline volume.  The result of $2{,}656\%$
(Table~\ref{tab:variance_results}) confirms that the manifold topology is rich enough
to accommodate diverse specialization patterns.  \emph{Importantly}, high topological
capacity proves only that collapse is absent — not that the ODE amplifies the correct
specialization.  Routing quality depends on the trained $f_\theta$ and is left to future
work, as discussed in Section~\ref{sec:conclusion}.
